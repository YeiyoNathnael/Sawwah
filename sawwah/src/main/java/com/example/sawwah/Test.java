...
public class Test {
    ...
    private static final EventManager system = new SawwahTree();
    ...
    private static void testAddEvents() {
        ...
        try {
            ...
            Event newEvent = new Event("Christmas Festival", start, end, "Dubai", "Cultural", 4);
            system.addEvent(newEvent);
            System.out.println("✓ Added new event: " + newEvent.name);

            // Try to add duplicate ID (should be ignored)
            Event duplicate = new Event("Duplicate Test", start, end, "Abu Dhabi", "Cultural", 3);
            ...
            Event sameName = new Event("Christmas Festival", LocalDateTime.parse("2025-12-26T14:00", formatter),
    ...
    private static void testSearchEvents() {
        ...
        try {
            ...
            Event found = system.searchEvent("National Day");
            if (found != null) {
            ...
            Event notFound = system.searchEvent("NonExistentEvent");
            if (notFound == null) {
            ...
            List<Event> duplicates = ((SawwahTree) system).getEventsByName("Christmas Festival");
    ...
    private static void testRemoveEvents() {
        ...
        try {
            ...
            List<Event> festivals = ((SawwahTree) system).getEventsByName("Christmas Festival");
            if (festivals.size() >= 1) {
                String idToRemove = festivals.get(0).id;
                boolean removedById = ((SawwahTree) system).removeEventById(idToRemove);
                if (removedById) {
    ...
    private static void testShowStats() {
        ...
        try {
            ...
            List<Event> trending = dm.getTrendingEventsEvents(allEvents, 3);
    ...
    private static void testBackup() {
        ...
        try {
            ...
            boolean success = DataManager.saveBackup(data, backupPath);
            if (success) {
            ...
            // Test loading backup
            DataManager dm = new DataManager();
            List<Event> loadedEvents = dm.loadBackup(backupPath);
